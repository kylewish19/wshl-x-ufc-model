...
def main():
    raw_dir = ROOT / "build/menifield_iwo_rerun/raw"
    ...
    report = {
        ...
        "fight": "Alonzo Menifield vs Iwo Baraniewski",
        "rerun_type": "ISOLATED_ODDS_BLIND_RECHECK",
        "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "trials": TRIALS,
        "winner": {
            "Alonzo Menifield": p_a,
            "Iwo Baraniewski": 1.0 - p_a,
        },
        "conditional_method_if_menifield_wins_baseline": base_a,
        "conditional_method_if_iwo_wins_baseline": base_b,
        "conditional_method_if_menifield_wins_v027_shadow": shadow_a,
        "conditional_method_if_iwo_wins_v027_shadow": shadow_b,
        "joint_outcomes_baseline": joint,
        "joint_simulation_10000": sim,
        "timing": timing,
        "evidence_density": {
            "menifield_prior_ufc_bouts": int(a["prior_bouts"]),
            "iwo_prior_ufc_bouts": int(b["prior_bouts"]),
            "menifield_days_since_last": float(a["days_since_last"]),
            "iwo_days_since_last": float(b["days_since_last"]),
            "menifield_age": float(a["age_years"]),
            "iwo_age": float(b["age_years"]),
            "menifield_reach": float(a["reach_inches"]),
            "iwo_reach": float(b["reach_inches"]),
        },
        "governance": {
            "odds_used_as_features": False,
            "official_stage_a_lock_unchanged": True,
            "purpose": "isolated diagnostic rerun requested by user",
        },
    }

    out = ROOT / "reports/ufc331_2026-09-19"
    out.mkdir(parents=True, exist_ok=True)
    (out / "menifield_iwo_isolated_rerun.json").write_text(
