...
def main():
    raw_dir = ROOT / "build/aswell_yoo_rerun/raw"
    ...
    report = {
        ...
        "fight": "Michael Aswell Jr. vs JooSang Yoo",
        "rerun_type": "ISOLATED_ODDS_BLIND_RECHECK",
        "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "trials": TRIALS,
        "winner": {
            "Michael Aswell Jr.": p_a,
            "JooSang Yoo": 1.0 - p_a,
        },
        "conditional_method_if_aswell_wins_baseline": base_a,
        "conditional_method_if_yoo_wins_baseline": base_b,
        "conditional_method_if_aswell_wins_v027_shadow": shadow_a,
        "conditional_method_if_yoo_wins_v027_shadow": shadow_b,
        "joint_outcomes_baseline": joint,
        "joint_simulation_10000": sim,
        "timing": timing,
        "evidence_density": {
            "aswell_prior_ufc_bouts": int(a["prior_bouts"]),
            "yoo_prior_ufc_bouts": int(b["prior_bouts"]),
            "aswell_days_since_last": float(a["days_since_last"]),
            "yoo_days_since_last": float(b["days_since_last"]),
            "aswell_age": float(a["age_years"]),
            "yoo_age": float(b["age_years"]),
            "aswell_reach": float(a["reach_inches"]),
            "yoo_reach": float(b["reach_inches"]),
        },
        "governance": {
    ...
    (out / "aswell_yoo_isolated_rerun.json").write_text(
