...
FIGHT = {
    ...
    "scheduled_seconds": 1500,
}
SEED = 3311911
TRIALS = 10_000

def main():
    raw_dir = ROOT / "build/arman_ruffy_rerun/raw"
    ...
    timing = {k: float(v) for k, v in timing_model.market_probabilities(tx, 1500).items()}

    report = {
        "event":"UFC 331: Van vs Pantoja 2",
        "fight":"Arman Tsarukyan vs Mauricio Ruffy",
        "rerun_type":"ISOLATED_ODDS_BLIND_RECHECK",
        "generated_at_utc":datetime.now(timezone.utc).isoformat(),
        "trials":TRIALS,
        "winner":{"Arman Tsarukyan":p_a,"Mauricio Ruffy":1.0-p_a},
        "conditional_method_if_arman_wins_baseline":base_a,
        "conditional_method_if_ruffy_wins_baseline":base_b,
        "conditional_method_if_arman_wins_v027_shadow":shadow_a,
        "conditional_method_if_ruffy_wins_v027_shadow":shadow_b,
        "joint_outcomes_baseline":joint,
        "joint_simulation_10000":sim,
        "timing":timing,
        "evidence_density":{
            "arman_prior_ufc_bouts":int(a["prior_bouts"]),
            "ruffy_prior_ufc_bouts":int(b["prior_bouts"]),
            "arman_days_since_last":float(a["days_since_last"]),
            "ruffy_days_since_last":float(b["days_since_last"]),
            "arman_age":float(a["age_years"]),
            "ruffy_age":float(b["age_years"]),
            "arman_reach":float(a["reach_inches"]),
            "ruffy_reach":float(b["reach_inches"])
        },
    ...
    (out/"arman_ruffy_isolated_rerun.json").write_text(json.dumps(report,indent=2,sort_keys=True),encoding="utf-8")
